---
title: "Activity 8: Revisiting the Martian Alphabet"
subtitle: "One Proportion Goodness-of-fit"
...
\key{There would be a 50\% chance of correctly identifying "Kiki" if everyone was randomly guessing.}
...
\key{Assuming 35 students in the class: $30\cdot 0.5 = 17.5$.}
...
\key{Assuming only one person incorrectly identified "Kiki":}
...
\key{$\frac{(34-17.5)^2}{1} = 15.56$ and $\frac{(1-17.5)^2}{1} = 15.56$}
...
obs_xsq <- (34-17.5)^2/17.5 + (1-17.5)^2/17.5
obs_xsq
```
...
\key{We will always obtain a positive $X^2$ statistic value since we are squaring the numerator and will always have a positive expected value.}
...
**Step 1:** Specify what value of $p$ is assumed under $H_0$: \key{0.5}

\vspace{0.25in}

**Step 2:** Specify how many **trials** should be used when flipping a coin: \key{35}

\vspace{0.25in}

**Step 3:** Specify which response goes with each side of a coin: \key{Heads = "Correct Guess", Tails = "Incorrect Guess"}

\vspace{0.25in}

**Step 4:** Flip a coin (or simulate flipping a coin) the number of times specified in Step 2. Tabulate how many responses went in each category: \key{Simulated during class.}
...
\key{$\frac{(16-17.5)^2}{17.5}+\frac{(19-17.5)^2}{17.5}=0.257$}

```{r include = F}
sim_xsq <- (16-17.5)^2/17.5 + (19-17.5)^2/17.5
sim_xsq
```
...
get_null_X2(1000) %>% 
  ...
  theme_bw() + 
  scale_x_continuous(limits = c(-1,40), breaks = seq(0,35,5))
...
get_null_X2(1000) %>% 
  ...
  theme_bw() + 
  scale_x_continuous(limits = c(-1,40), breaks = seq(0,35,5)) +
  geom_vline(xintercept = obs_xsq, color = "red") +
  geom_text(label = paste("X^2 =", round(obs_xsq,3)), color = "red", x = obs_xsq, y = 0, hjust = -0.25)
```
...
\key{SInce our p-value falls far to the right of the chi-square distribution, our p-value is estimated to be about 0 or (<0.0001).}

\vspace{0.1in}

**11. Based on your p-value, what would you conclude for your null and alternative hypotheses from #2 and #3?**

\key{Since our p-value is small (less than 0.05), we would reject the null hypthesis that there is an equal chance of correctly and incorrectly guessing and conclude there is evidence the probability of correctly identifying "Kiki" is not equal to 50\%.}
...
\key{The observations are independent because each student's response does not tell us information about another student's response. Therefore, independence is met. We do have a "large enough" sample size since there are more than 5 expected students for both cells (expected 17.5 students to correctly identified "Kiki" and expected 17.5 students to incorrectly identify "Kiki"). Therefore, the large sample size condition is met.}
...
pchisq(31.114, df = 1, lower.tail = FALSE)
```

\vspace{0.25in}

Running the code you just wrote in `R` gave me a p-value of `r pchisq(7.36, df = 1, lower.tail = FALSE)`. 

**14. Based on this p-value what conclusion would you reach regarding the null hypothesis?**

\key{With a p-value < 0.0001 (2.43 x $10^{-8}$) we would reject the null hypothesis that there is an equal probability of correctly and incorrectly guessing.}

\vspace{0.1in}

**15. Did you reach similar conclusions using simulation-based methods? Why do you believe that is the case?**

\key{Yes, our conclusion with the simulation based method is similar to that of the theory based method. They should be simular because the theory based method is using a $\chi^2$ distribution to approximate the simulated null distribution of simulated $X^2$ statistics from earlier.}
...
```{r}
knitr::include_graphics("images/fire_schematic.png")
```
...
|----------------|-------|--------------------------|
| Inner Burn     | 520   | `r 520/3000`             |
| Inner Edge     | 210   | 0.070                    |
| Outer Edge     | 240   | `r 240/3000`             |
...
\key{$H_A$: At least one of the proportions $(p_i)$ differs from the proportion based on total area / size of the region.}
...
|----------------|-----------------|-----------------|
| Inner Burn     | 2               |`r 75*(520/3000)`|
| Inner Edge     | 12              |`r 75*(210/3000)`|
| Outer Edge     | 18              |`r 75*(240/3000)`|
| Outer Unburned | 43              |`r 75*(2030/3000)`|
...
\key{$\frac{(2-13)^2}{13}+\frac{(12-5.25)^2}{5.25}+\frac{(18-6)^2}{6}+\frac{(43-50.75)^2}{50.75}=43.17$}

```{r, include = F}
(2-13)^2/13 + (12-5.25)^2/5.25 + (18-6)^2/6 +(43-50.75)^2/50.75
```
...
\key{We should use a $\chi^2$ with 4 regions - 1 degrees of freedom.}
...
\key{The p-value of less than 0.0001 is less than the alpha threshold cuttoff of 0.05; therefore we reject the null hypothesis that the deer show no preference for any specific region and the frequencies of dear are proportional to the size of the region.}
