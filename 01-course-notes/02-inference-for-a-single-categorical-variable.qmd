...
In this chapter, we will discuss methods for making decisions concerning research questions involving only a one proportion. Even though we'll be adding lots of terminology along the way, we will use the same logical approach to answering questions that was introduced in Chapter 1.
...
+--------------------------------------------------------------+-------------------------------------------------------------------------------+
| Example 1.3: Are women passed over for managerial training?\ | Is there statistical evidence for sex discrimination against females?\     |
+--------------------------------------------------------------+-------------------------------------------------------------------------------+
...
| \                                                                             |                                                                                                  |
+-------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------+
| Is there statistical evidence for sex discrimination against females?\     | H~0~: The selection process is fair.\                                                            |
| \                                                                             | \                                                                                                |
...
Is there statistical evidence for sex discrimination against females?
...
-   If the p-value is above 0.10, we *do have not have enough evidence* to support the research question
:::
...
helperhinder_binom |> 
  ...
  geom_text(aes(label = round(dbinom, 3)*100), vjust = -0.6, color = "gray30", size = 3) +
  labs(x = "Number of 10-month olds who chose 'Helper'",
       y = "Binomial Probability") +
  scale_x_continuous(breaks = seq(1,16)) +
  scale_y_continuous(breaks = seq(0,0.25, 0.05), limits = c(0,0.25), labels = scales::percent) +
  theme_bw() +
...
acupuncture_binom |> 
  ggplot(aes(x = x, y = dbinom)) +
  geom_segment(aes(xend = x, yend = 0), size = 1) +
  geom_text(aes(label = round(dbinom, 2)*100), vjust = -0.5, color = "gray30", size = 3) +
  labs(x = "Proportion that get relief",
       y = "Binomial Probability") +
  scale_x_continuous(limits = c(0.72, 1), breaks = seq(0.7, 1, 0.02)) +
  scale_y_continuous(breaks = seq(0,0.15, 0.05), limits = c(0,0.15), labels = scales::percent) +
  theme_bw()+
  theme(aspect.ratio = 0.5)
```
...
#### Approach 3: Normal Approximation (https://shiny.stat.ncsu.edu/jbpost2/NormalApproximation/)
...
::: callout-note
### Big Picture: Meaning of 95% Confidence
