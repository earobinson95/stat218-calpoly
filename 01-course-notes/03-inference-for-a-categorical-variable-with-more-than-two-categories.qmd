...
Using the data, we can determine the proportion of fires occurring in each season are given below:
...
sim |> 
  ...
  labs(title = "Simulated under the assumption that the seasonal distribution of fire\nin Riverside County matches the seasonal distribution of fires across California", 
...
15. The Test Statistic for the observed data from our 146 fires in Riverside County is $X^2=17.98$. Is this value consistent with results we would expect to see if Fires are equally dispersed over the four seasons? Explain.
...
chisq_test(x = riverside_incidents,
           response = Season,
           p = c(0.219, 0.112, 0.596, 0.073) # note this goes in alphabetical order (Fall, Spring, Summer, Winter)
           )
```
