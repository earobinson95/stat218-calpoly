...
+ <span style="color:blue;">Explanatory: Vesting Group (categorical) -- Vested Interest / No Vested Interest</span>
+ <span style="color:blue;">Response: Success (categorical) -- Achieved Success / Did not achieve success</span>

2.  Convert the research question into your null and alternative hypotheses.

+ <span style="color:blue;">Null: Whether the observer has a vested interest or not **does not** have an impact on performance on a skill-based task.</span>
+ <span style="color:blue;">Alternative: Whether the observer has a vested interest or not **has** an impact on performance on a skill-based task.</span>

3. Is it appropriate to use the Chi-square distribution or do we need to use simulation?

+ <span style="color:blue;">Independence: Whether one subject achieves success or not does not tell us whether or not another person achieves success or not. </span>
+ <span style="color:blue;">Expected counts > 5: The smallest expected count is $\frac{11}{24}(12) = 5.5$ for the Vested Interest group. </span>

<span style="color:blue;">It would be appropriate to use the chi-square distribution to approximate the simulated null sampling distribution. Simulation will still work, too!</span>
...
<span style="color:blue;">Given there is no relationship between vested interest and success, there is a 6% chance of observing a chi-square test statistic of 4.196 or greater.</span>

<span style="color:blue;">p-value = 0.06 < $\alpha$-cutoff of 0.10 -> Reject the Null</span>
...
<span style="color:blue;">At an $\alpha = 0.01$, we have evidence to conclude that whether the observer has a vested interest or not **has** an impact on performance on a skill-based task.</span>
...
<span style="color:blue;">We would be confounding skill with having a vested interest or not.</span>

<span style="color:blue;">Oddly, $\hat p_{success | vested} = 3/12=0.25$ and $\hat p_{success | not vested} = 8/12=0.0.66$.</span>

<span style="color:blue;">I should have flipped the groups that were Mario Cart pros -- we don't know whether the skill led to a win or whether the vested interest led to a win.</span>
...
<span style="color:blue;">By randomly assigning subjects to either the vested or no vested groups, we balance the skill out between the two groups so the groups have similar characteristics outside of the vested group helping us determine a cause-and-effect relationship.</span>
...
<span style="color:blue;">Yes, since we randomly assigned the vested interest group we can assume any confounding variables are balanced between the two groups (e.g., skill).</span>
...
+ Observational Unit: <span style="color:blue;">An Oklahoma City Thunder NBA home game during the 2008-2009 season</span>

\vspace{0.15in}

+ Explanatory Variable: <span style="color:blue;">Crowd (categorical) - sell out / smaller</span>

\vspace{0.15in}

+ Response Variable: <span style="color:blue;">Game outcome (categorical) - won / lost</span>
...
$$\hat p_{win|sell-out} = 3/18=0.166$$

\vspace{0.15in}

$$\hat p_{win|smaller} = 12/23 = 0.522$$

\vspace{0.15in}

4. Do the two variables appear to be associated? Explain.

<span style="color:blue;">Yes, it appears that the probability of winning with a smaller crowd is about five times as large as the probability of winning with a sell-out crowd.</span>
...
<span style="color:blue;">The strength of the opposing team would make for a better competition meaning it might be more likely the game sells out, but also it might be more likely the team loses because it is a tougher game.</span>
...
<span style="color:blue;">The strength of the opposing team.</span>
...
$$\hat p_{sell-out|strong} = 13/22=0.591$$


$$\hat p_{sell-out|weak} = 5/19=0.263$$

<span style="color:blue;">The probability they sell out against a strong team (59.1%) is a little more than twice as likely they sell out against a weak team (26.3%).</span>
...
$$\hat p_{win|strong} = 4/22=0.182$$


$$\hat p_{win|weak} = 11/19 = 0.579$$

<span style="color:blue;">The probability they win against a strong team (18.2%) is under half as likely they win against a weak team (57.9%).</span>
...
<span style="color:blue;">The confounding variable of the strength of opponent appears to be related to the probability of selling out and the probability of winning. Thus, we can't separate the strength of the opponent from the crowd size.</span>
...
<span style="color:blue;">It appears the chance of winning against a sell out crowd (16.67%) is much less than the chance of winning against a smaller crowd (52.2%), however, we cannot determine the crowd size causes the chance of winning since the strength of the opposing team appears highly related to both the chance of selling out and the chance of winning.</span>
