...
2.  **What are the variables in this study? Are the categorical or numeric?**

Your answer goes here.

# Visualizing One Categorical Variable

Let's start by exploring how many matches in our data set were `won` by each uniform color. Notice we are not interested in which sport was competed in, only the color of the uniform.

3.  **Create a *stacked bar plot* of the counts for the variable `won`.**

**Step 1:** Set-up the data and color by the variable(s) of interest: `fill = VARIABLE`

**Step 2:** Define what geometric object should be plotted on the plot -- now we're using bars! `geom_bar()`

**Step 3:** Declare an informative y-axis label: `labs(y = "WHAT IS ON THE Y-AXIS?")`

Fill out the missing parts of the code below

```{r freq-barplot}

ggplot(data = __________, 
       mapping = aes(x = "Observed Results",
                     fill = _______
                     )
       ) + 
  geom_bar(stat = "count",
  ...
  labs(y = "___________________")
```

We could also choose to display the data as a proportion in a ***filled bar plot***. To find the proportions, we divide the count in each level by the total sample size. The resulting values are sample proportions (your $\hat p$'s if you will).

To get *proportions* instead of *counts* we need to change the position of the bars from `position = stack` to `position = fill`.

4.  **Use this new code to modify the code you had previously. Copy-and-paste your code from the stacked bar plot above and change plot proportions instead of counts. Make sure to change your y-axis label to match proportions rather than counts!**

```{r relative-freq-barplot}

# Copy-and-paste + modify your code from above
...
The other part of Exploratory Data Analysis (EDA) is making summary statistics. For a categorical variable, this means making tables of counts and tables of proportions. Let's explore how to do that.
...
-   `mutate()`, which modifies our data set by adding new variables or changing existing variables
...
Run the code below. Discuss in your group where the values 248, 457, and 0.5 came from.

```{r binom-test-1}
binom.test(x = 248, n = 457, p = 0.5, alternative = "greater")
```

10. **Based on the output above, what decision (Reject the Null or Fail to Reject the Null) do you make about your hypotheses? Justify your answer using the p-value.**
...
13. **Copy and change the `binom.test()` code above to conduct a two-sided test.**
...
-   Now suppose instead of 248, we found that 261 matches were won by competitors wearing read (i.e., an observe a statistic of 0.571).

15. **How does the distance between our observed statistic and hypothesized null value change? It might be helpful to draw a picture.**
...
**Research Question:** Do *boxing* competitors that wear red uniforms win a majority of the time?
